...
//    const int m = 5;
    ...
    randMatrix(matrixB);
    endTime = clock();
    cout << "The run time of generate matrix is: " << (double) (endTime - startTime) / CLOCKS_PER_SEC << "s" << endl;

    startTime = clock();
    vector<vector<float>> matrixE = multiplyMatrixByBlas(matrixA, matrixB);
    endTime = clock();
    cout << "The run time of openBLAS matrix multiply is: " << (double) (endTime - startTime) / CLOCKS_PER_SEC << "s"
         << endl;

    startTime = clock();
    vector<vector<float>> matrixC = multiplyMatrixNormal(matrixA, matrixB);
    endTime = clock();
    cout << "The run time of Normal matrix multiply is: " << (double) (endTime - startTime) / CLOCKS_PER_SEC << "s"
         << endl;

    startTime = clock();
    vector<vector<float>> matrixD = multiplyMatrixAdvance(matrixA, matrixB);
    endTime = clock();
    cout << "The run time of Advance matrix multiply is: " << (double) (endTime - startTime) / CLOCKS_PER_SEC << "s"
         << endl;

    bool flag = verifyTwoMatrixAreEqual(matrixC, matrixD);
    cout << "The result(C D) of multiply on A, B is equal?:" << flag << endl;
    flag = verifyTwoMatrixAreEqual(matrixC, matrixE);
    cout << "The result(C E) of multiply on A, B is equal?:" << flag << endl;
